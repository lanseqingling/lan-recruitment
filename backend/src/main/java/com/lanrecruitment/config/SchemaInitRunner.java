...
public class SchemaInitRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS sys_user (" +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS tag (" +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS resume (" +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS resume_tag (" +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS job (" +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS job_tag (" +
                "id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT 'ID'," +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS job_apply (" +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS match_score (" +
                "id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT 'ID'," +
                "resume_id BIGINT NOT NULL COMMENT '简历ID'," +
                "job_id BIGINT NOT NULL COMMENT '岗位ID'," +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS notice (" +
        ...
        ddls.add("CREATE TABLE IF NOT EXISTS login_log (" +
        ...
        try (Connection conn = dataSource.getConnection(); Statement stmt = conn.createStatement()) {
            ...
            tryExecute(stmt, "ALTER TABLE sys_user ADD COLUMN audit_status TINYINT DEFAULT 1 COMMENT '审核状态：0待审核 1通过 2拒绝（HR/岗位审核用）'");
            tryExecute(stmt, "ALTER TABLE sys_user ADD COLUMN real_name VARCHAR(20) COMMENT '真实姓名'");
            tryExecute(stmt, "ALTER TABLE sys_user ADD COLUMN phone VARCHAR(20) COMMENT '手机号'");

            tryExecute(stmt, "ALTER TABLE resume ADD COLUMN city VARCHAR(50) COMMENT '所在城市'");
            tryExecute(stmt, "ALTER TABLE resume ADD COLUMN expect_job_type VARCHAR(50) COMMENT '期望岗位类型'");
            tryExecute(stmt, "ALTER TABLE resume ADD COLUMN expect_salary VARCHAR(50) COMMENT '期望薪资'");
            tryExecute(stmt, "ALTER TABLE resume ADD COLUMN file_name VARCHAR(255) COMMENT '附件简历文件名'");
            tryExecute(stmt, "ALTER TABLE resume ADD COLUMN file_url VARCHAR(255) COMMENT '附件简历访问URL'");
            tryExecute(stmt, "ALTER TABLE resume ADD COLUMN file_type VARCHAR(20) COMMENT '附件简历类型（pdf/jpg/png）'");
            tryExecute(stmt, "ALTER TABLE resume ADD COLUMN file_size BIGINT COMMENT '附件简历大小（字节）'");
            tryExecute(stmt, "ALTER TABLE resume ADD COLUMN is_default TINYINT DEFAULT 0 COMMENT '是否默认简历：1是 0否'");

            tryExecute(stmt, "ALTER TABLE job ADD COLUMN audit_status TINYINT DEFAULT 0 COMMENT '审核状态：0待审核 1通过 2拒绝'");
        }
    }

    private void tryExecute(Statement stmt, String ddl) throws Exception {
        try {
            stmt.execute(ddl);
